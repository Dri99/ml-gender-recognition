...
def validate_GMM(DTR, LTR, covType, NG, pi, m, gauss, k=5):
    ...
    if m < 12:
        print("%s\t-\tminDCF : %1.3f \tDCF : %1.3f \t \u03C0_eff : %1.1f \t Ng = %d \tm = %d" % (
            covType, minDCF, DCF, pi, NG, m))
    else:
        print("%s\t-\tminDCF : %1.3f \tDCF : %1.3f \t \u03C0_eff : %1.1f \t Ng = %d \t Raw Data" % (
            covType, minDCF, DCF, pi, NG))
...
def logpdf_GMM(X, gmm):
    ...
    for i, g in enumerate(gmm):
        ...
        S[i, :] = numpy.log(w) + mvg.logpdf_GAU_ND(X, mu,C)  # we can interpret Gaussian components G as representing sub-classes (i.e. clusters) of our data
        # print("Fast: %f Normal: %f ", (mvg.logpdf_GAU_ND_fast(X, mu,C),mvg.logpdf_GAU_ND(X, mu,C),))
    return S
...
def M_step(Stats, covType, psi=0.01):
    ...
    for g in gmm:
        ...
        s[s < psi] = psi
        g[2] = numpy.dot(U, mvg.mcol(s) * U.T)
...
def plot_GMM(DTR, LTR):
    ...
    plt.bar(br1, minDCF_ct[0, :], label="minDCF($\widetilde{\pi}$ = 0.5) - Full-Cov", color='gold', linewidth=.5,
            width=barWidth, edgecolor='black')
    plt.bar(br2, minDCF_ct[1, :], label="minDCF($\widetilde{\pi}$ = 0.5) - Diag-Cov", color='darkorange', linewidth=.5,
            width=barWidth, edgecolor='black')
    plt.bar(br3, minDCF_ct[2, :], label="minDCF($\widetilde{\pi}$ = 0.5) - Tied Full-Cov", color='red', linewidth=.5,
            width=barWidth, edgecolor='black')
    plt.bar(br4, minDCF_ct[3, :], label="minDCF($\widetilde{\pi}$ = 0.5) - Tied Diag-Cov", color='darkred',
            linewidth=.5, width=barWidth, edgecolor='black')
...
def evaluate_GMM(DTR, LTR, DTE, LTE, covType, NG, pi, m, gauss):
    ...
    for lab in set(LTR):
        mu, C = mvg.empirical_moments(DTR[:, LTR == lab])
        GMM_1 = [(1.0, mu, C)]
    ...
    minDCF = evaluation.compute_min_dcf(llr,LTE, 0.5, 1, 1)
